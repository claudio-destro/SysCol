...
describe("PAR response parsing", () => {
  const makeTest = <K extends keyof typeof REGISTERS>(key: K): ProvidesCallback => {
    ...
    return () => {
      const args0 = rw ? [{key: register}, {key: rw, value: params[0]}] : [{key: register, value: params[0]}];
      expect(parseCommandResponse(`{SC,PAR,${key}:${params.join(",")}}`)).toMatchObject({
        command: "par",
        argv: [{key: test}, ...args0, ...params.slice(1).map(key => ({key, value: undefined}))],
        error: false,
      } satisfies Omit<CommandResponse, "commandLine">);
    };
  };
  test("should parse response 7.3,G", makeTest("7.3,G"));
  test("should parse response 7.3,E", makeTest("7.3,E"));
